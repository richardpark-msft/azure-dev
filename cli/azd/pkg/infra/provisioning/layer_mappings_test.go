...
func TestNewProviderEnvironmentManagerAppliesInputMappings(t *testing.T) {
	t.Parallel()
	...
	manager, mapped := newProviderEnvironmentManager(&mockenv.MockEnvManager{}, env, options)
	providerEnv := manager.environment()
...
func TestProviderEnvironmentManagerFiltersInputAliases(t *testing.T) {
	t.Parallel()
	...
	manager := &providerEnvironmentManager{
		Manager:       inner,
		baseEnv:       baseEnv,
